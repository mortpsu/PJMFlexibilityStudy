...
KLOW(K) /k0*k2/
N_23_2(N) Subset of time period (23 to 02) /2,3,24,25/
N_3_6(N) Subset of time period (03 to 06) /4,5,6,7/
N_7_10(N) Subset of time period (07 to 10) /8,9,10,11/
N_11_14(N) Subset of time period (11 to 14) /12,13,14,15/
N_15_18(N) Subset of time period (15 to 18) /16,17,18,19/
N_19_22(N) Subset of time period (19 to 22) /20,21,22,23/
...
GTINY(G) Very small CTs and ICs        /g84*g91,g94*g121,g125*g129,g214*g245/
GSLOW(G) Slower starting units         /g1*g213/
GFAST(G) GTs and ICs                   /g81*g149,g214*g245/
...
*GOFF(G)   Temporary offline units       /g83,g86,g87,g89*g91,g96,g97,g100*g103,g105,g107*g109,g112*g115,g118*g121,g123,g126,g134,g138*g141,g143*g145,g149,g215*g216,g222*g230,g236*g245/
...
vRS10_SHORT(J,N)    Volume not met of 10 min spinning reserves
vRN10(G,N)          Amount of 10 min non-spinning reserves provided (MW)
vRP10_SHORT(J,N)    Volume not met of 10 min primary reserves

* Variables for PJM ORDC curves (30 minutes)
vRS30(G,N)          Amount of 30 min spinning reserves provided (MW)
*vRS30_SHORT(N,J)    Volume not met of 30 min spinning reserves
vRN30(G,N)          Amount of 30 min non-spinning reserves provided (MW)
vRP30_SHORT(J,N)    Volume not met of 30 min primary reserves
;

* Set stepsize for ORDC - Spinning Reserves (10 min)
vRS10_SHORT.UP(J,N)$[ORD(J) LT CARD(J)] = pRS10_BlockSize(J,N) ;
vRP10_SHORT.UP(J,N)$[ORD(J) LT CARD(J)] = pRP10_BlockSize(J,N) ;

* Set stepsize for ORDC - Primary Reserves (30 min)
vRP30_SHORT.UP(J,N)$[ORD(J) LT CARD(J)] = pRP30_BlockSize(J,N) ;
...
*Enforce total system requirement for spinning reserves 10 min
eReqSR10_23_2(N)
eReqSR10_3_6(N)
eReqSR10_7_10(N)
eReqSR10_11_14(N)
...
*Enforce total system requirement for Primary reserves 10 min
eReqRP10_23_2(N)
eReqRP10_3_6(N)
eReqRP10_7_10(N)
eReqRP10_11_14(N)
...
*Enforce total system requirement for Primary reserves 30 min
eReqRP30_23_2(N)
eReqRP30_3_6(N)
eReqRP30_7_10(N)
eReqRP30_11_14(N)
...
*eCost..           z =E=(vGenCost  + vPenaltyCost);
...
eReserveCost..    vReserveCost =E= sum((J,N), vRS10_SHORT(J,N)*pRS10_Price(J,N))+sum((J,N), vRP10_SHORT(J,N)*pRP10_Price(J,N))+sum((J,N), vRP30_SHORT(J,N)*pRP30_Price(J,N));
...
eSG(G,N)..           sum(k, pwl(G,N,k)) =e= vPower(G,N) ;
...
eGenFuelCost(G,N)$GCOMMIT(G)..  vGenFuelCost(G,N)  =e= sum(k$[ord(k) >2],pwl(G,N,k)*pIncrFuelCost(K,G))
...
*Setting the requirements for interval (23-2)

eReqSR10_23_2(N)$(N_23_2(N))..          sum(G, vRS10(G,N)) + sum(J,vRS10_SHORT(J,N)) =G= pReqRS10_23_2;

*Setting the requirements for interval (3-6)

eReqSR10_3_6(N)$(N_3_6(N))..          sum(G, vRS10(G,N)) + sum(J,vRS10_SHORT(J,N)) =G= pReqRS10_3_6;

*Setting the requirements for interval (7-10)

eReqSR10_7_10(N)$(N_7_10(N))..          sum(G, vRS10(G,N)) + sum(J,vRS10_SHORT(J,N)) =G= pReqRS10_7_10;

*Setting the requirements for interval (11-14)

eReqSR10_11_14(N)$(N_11_14(N))..          sum(G, vRS10(G,N)) + sum(J,vRS10_SHORT(J,N)) =G= pReqRS10_11_14;

*Setting the requirements for interval (15-18)

eReqSR10_15_18(N)$(N_15_18(N))..          sum(G, vRS10(G,N)) + sum(J,vRS10_SHORT(J,N)) =G= pReqRS10_15_18;

*Setting the requirements for interval (19-22)

eReqSR10_19_22(N)$(N_19_22(N))..          sum(G, vRS10(G,N)) + sum(J,vRS10_SHORT(J,N)) =G= pReqRS10_19_22;

*Requirement of the primary reserves for 10 minutes

*Setting the requirements for interval (23-2)

eReqRP10_23_2(N)$(N_23_2(N))..  sum(G, vRS10(G,N)) + sum(GTINY, vRN10(GTINY,N)) + sum(J,vRP10_SHORT(J,N)) =G= pReqRP10_23_2;

*Setting the requirements for interval (3_6)

eReqRP10_3_6(N)$(N_3_6(N))..  sum(G, vRS10(G,N)) + sum(GTINY, vRN10(GTINY,N)) + sum(J,vRP10_SHORT(J,N)) =G= pReqRP10_3_6;

*Setting the requirements for interval (7_10)

eReqRP10_7_10(N)$(N_7_10(N))..  sum(G, vRS10(G,N)) + sum(GTINY, vRN10(GTINY,N)) + sum(J,vRP10_SHORT(J,N)) =G= pReqRP10_7_10;

*Setting the requirements for interval (11-14)

eReqRP10_11_14(N)$(N_11_14(N))..  sum(G, vRS10(G,N)) + sum(GTINY, vRN10(GTINY,N)) + sum(J,vRP10_SHORT(J,N)) =G= pReqRP10_11_14;

*Setting the requirements for interval (15_18)

eReqRP10_15_18(N)$(N_15_18(N))..  sum(G, vRS10(G,N)) + sum(GTINY, vRN10(GTINY,N)) + sum(J,vRP10_SHORT(J,N)) =G= pReqRP10_15_18;

*Setting the requirements for interval (19_22)

eReqRP10_19_22(N)$(N_19_22(N))..  sum(G, vRS10(G,N)) + sum(GTINY, vRN10(GTINY,N)) + sum(J,vRP10_SHORT(J,N)) =G= pReqRP10_19_22;


*Contribution of unit g to Spinning Reserves  30 min
eCapLimRS30(G,N)$[ORD(N) > 1]..   vW(G,N) + vRS30(G,N) =L= (pGenData(G,'Pmax') - pGenData(G,'Pmin')) * vU(G,N);
eRampLimRS30(G,N)$[ORD(N) > 1]..  vRS30(G,N) =L= pGenData(G,'Ramp') * 30 * vU(G,N);

*Contribution of Non-Spinning Reserves 30 min
eLimRN30(G,N)$[ORD(N) > 1 and GFAST(G)]..          vRN30(G,N)=L= (1 - vU(G,N)) * pGenData(G,'Pmax') * (pAvail(G));
eLimRN30a(G,N)$[ORD(N) > 1 and not(GTINY(G))]..    vRN30(G,N)=L= 0;

*Requirement of the spinning reserves for 30 minutes
*eReqSR30(N)$[ORD(N) > 1]..          sum(G, vRS30(G,N)) + sum(J,vRS30_SHORT(N,J)) =G= pReqRS30;

*Requirement of the Reserves for 30 minutes

*Setting the requirements for interval (23-2)

eReqRP30_23_2(N)$(N_23_2(N))..  sum(G, vRS30(G,N)) + sum(GFAST, vRN30(GFAST,N)) + sum(J,vRP30_SHORT(J,N)) =G= pReqRP30_23_2;

*Setting the requirements for interval (3_6)

eReqRP30_3_6(N)$(N_3_6(N))..  sum(G, vRS30(G,N)) + sum(GFAST, vRN30(GFAST,N)) + sum(J,vRP30_SHORT(J,N)) =G= pReqRP30_3_6;

*Setting the requirements for interval (7_10)

eReqRP30_7_10(N)$(N_7_10(N))..  sum(G, vRS30(G,N)) + sum(GFAST, vRN30(GFAST,N)) + sum(J,vRP30_SHORT(J,N)) =G= pReqRP30_7_10;

*Setting the requirements for interval (11_14)

eReqRP30_11_14(N)$(N_11_14(N))..  sum(G, vRS30(G,N)) + sum(GFAST, vRN30(GFAST,N)) + sum(J,vRP30_SHORT(J,N)) =G= pReqRP30_11_14;

*Setting the requirements for interval (15_18)

eReqRP30_15_18(N)$(N_15_18(N))..  sum(G, vRS30(G,N)) + sum(GFAST, vRN30(GFAST,N)) + sum(J,vRP30_SHORT(J,N)) =G= pReqRP30_15_18;

*Setting the requirements for interval (19_22)

eReqRP30_19_22(N)$(N_19_22(N))..  sum(G, vRS30(G,N)) + sum(GFAST, vRN30(GFAST,N)) + sum(J,vRP30_SHORT(J,N)) =G= pReqRP30_19_22;
...
pPriceRS10(N)$(N_23_2(N))= eReqSR10_23_2.m(N);
pPriceRS10(N)$(N_3_6(N))= eReqSR10_3_6.m(N);
pPriceRS10(N)$(N_7_10(N))= eReqSR10_7_10.m(N);
pPriceRS10(N)$(N_11_14(N))= eReqSR10_11_14.m(N);
pPriceRS10(N)$(N_15_18(N))= eReqSR10_15_18.m(N);
pPriceRS10(N)$(N_19_22(N))= eReqSR10_19_22.m(N);

pPriceRP10(N)$(N_23_2(N))= eReqRP10_23_2.m(N);
pPriceRP10(N)$(N_3_6(N))= eReqRP10_3_6.m(N);
pPriceRP10(N)$(N_7_10(N))= eReqRP10_7_10.m(N);
pPriceRP10(N)$(N_11_14(N))= eReqRP10_11_14.m(N);
pPriceRP10(N)$(N_15_18(N))= eReqRP10_15_18.m(N);
pPriceRP10(N)$(N_19_22(N))= eReqRP10_19_22.m(N);

pPriceRP30(N)$(N_23_2(N))= eReqRP30_23_2.m(N);
pPriceRP30(N)$(N_3_6(N))= eReqRP30_3_6.m(N);
pPriceRP30(N)$(N_7_10(N))= eReqRP30_7_10.m(N);
pPriceRP30(N)$(N_11_14(N))= eReqRP30_11_14.m(N);
pPriceRP30(N)$(N_15_18(N))= eReqRP30_15_18.m(N);
pPriceRP30(N)$(N_19_22(N))= eReqRP30_19_22.m(N);

pShortRS10(N)$[ord(N) >1] = sum(J,vRS10_SHORT.L(J,N));
pShortRP10(N)$[ord(N) >1] = sum(J,vRP10_SHORT.L(J,N));
pShortRP30(N)$[ord(N) >1] = sum(J,vRP30_SHORT.L(J,N));

pTotalRS10(N)$[ord(N) >1] = sum(G, vRS10.L(G,N));
pTotalRN10(N)$[ord(N) >1] = sum(G, vRN10.L(G,N));
pTotalRP10(N)$[ord(N) >1] = sum(G, vRS10.L(G,N)) + sum(GTINY, vRN10.L(GTINY,N));
pTotalRP30(N)$[ord(N) >1] = sum(G, vRS30.L(G,N)) + sum(GFAST, vRN30.L(GFAST,N));
...
display Z.L, vGenCost.L, vStartupCost.L, vReserveCost.L, vPenaltyCost.L, pGenCost, pGenSUCost, pFuelGen, pTechGen, vPower.l, vRS10.l, vRN10.l, vRS30.l, vRN30.l, vW.l , vU.l , vUP.l,vDown.l,vSU_TYPE.l,vNse.l,vOverGen.L,pShortRS10, pShortRP10, pShortRP30, pAVGcost,pPrice,pPriceRS10,pPriceRP10,pPriceRP30,vGenFuelCost.l,pVOMCost,pMIPGAP,pModelStat,pTotalRS10,pTotalRN10,pTotalRP10,pTotalRP30;

execute_unload "results_base.gdx" Z.L, vGenCost.L, vStartupCost.L, vReserveCost.L, vPenaltyCost.L, pGenCost, pGenSUCost, pFuelGen, pTechGen, vPower.l, vRS10.l, vRN10.l, vRS30.l, vRN30.l, vW.l , vU.l , vUP.l,vDown.l,vSU_TYPE.l,vNse.l,vOverGen.L,pShortRS10, pShortRP10, pShortRP30, pAVGcost,pPrice,pPriceRS10,pPriceRP10,pPriceRP30,vGenFuelCost.l,pVOMCost,pMIPGAP,pModelStat;
