...
filename = sprintf('./forney_results/results_%d_%d.xlsx',day1.Month,day1.Day);
...
for i =1:6
    writetable([results(1).AVGcost(:,1),array2table(results(1).AVGcost(:,2:end).Variables .* results(1).Gen(:,2:end).Variables)],filename,'Sheet',sprintf("gen_cost_%s",results(i).model_name),'Range','A3');
end
case_names = {'Index'};
for c = 1:6 ; case_names = [case_names, results(c).model_name]; end
...
names = {};
for i=1:6;names(i) = {sprintf('%s_case',results(i).model_name(9:end))};end
for j=[1,5];
    data(j).Case = names(j);
    data(j).Pmax = 919;
    data(j).Pmin = 367.6; 
